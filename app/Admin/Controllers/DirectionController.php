...
class DirectionController extends Controller
{
    ...
    public function index(Content $content)
    {
        ...
        return $content
            ->header('Блок Каталог на странице '.$this->page_name)
            ->description(' список ')
    ...
    public function create(Content $content)
    {
        ...
        return $content
            ->header('Блок Направления на странице '.$this->page_name)
...
//        $grid->id('Id');
        ...
        $grid->items('Фото')->display(function ($items) {
            $count = count($items);
            return "<a href='/admin/direction_items?set={$this->id}' title='перейти к слайдам'><span class='label label-warning'>{$count}</span></a>";
        });
        return $grid;
    }
