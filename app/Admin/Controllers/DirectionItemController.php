...
class DirectionItemController extends Controller
{
    ...
    public function create(Content $content)
    {
        ...
        return $content
            ->header('Фото на странице '.$this->page_name)
            ->description(' список ')
            ->body($this->form());
    }
    ...
    {
        $grid = new Grid(new DirectionItem);

        $grid->model()->where('direction_id', session('direction_id'));
...
//        $grid->updated_at('Updated at');
    ...
    {
        $show = new Show(DirectionItem::findOrFail($id));
    ...
    {
        $form = new Form(new DirectionItem);

        $form->hidden('direction_id')->value(session('direction_id'));
